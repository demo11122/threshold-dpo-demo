...
from diffusers.utils import check_min_version, deprecate, is_wandb_available, make_image_grid, numpy_to_pil
...
def parse_args():
    ...
    parser.add_argument("--dpo_type", type=str, default='regular', choices=['regular', 'offset', 'leveled_offset', 'beta_offset'])
    parser.add_argument("--train_method", type=str, default=None, choices=['sft', 'dpo', 'draft'])
    
    parser.add_argument("--offset_alpha", type=float, default=100)
...
def main():
    ...
    if args.train_method == 'dpo' or args.train_method =='draft': ref_unet.requires_grad_(False)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            ...
            if len(models) > 1:
                assert args.train_method == 'dpo' or 'draft'# 2nd model is just ref_unet in DPO case
    ...
    if args.train_method == 'dpo' or args.train_method == 'draft':
        ...
        if args.choice_model:
            ...
            def do_flip(jpg0, jpg1, prompt, score_return=False):
                # if score return is True, return the score difference as well in absolute value
                scores = selector.score([jpg0, jpg1], prompt)
                print(scores)
                if not score_return:
                    return scores[1] > scores[0]
                else:
                    return scores[1] > scores[0], torch.abs(torch.Tensor([scores[1]-scores[0]]))
            def choice_model_says_flip(batch, score_return=False):
                assert len(batch['caption'])==1 # Can switch to iteration but not needed for nwo
                return do_flip(batch['jpg_0'][0], batch['jpg_1'][0], batch['caption'][0], score_return)
    ...
    if args.sdxl:
        ...
        if args.train_method == 'dpo':
            ref_unet.to(accelerator.device, dtype=weight_dtype)
            print("offload ref_unet")
            ref_unet = accelerate.cpu_offload(ref_unet, offload_buffers=True)
    ...
    if os.path.exists("score_diff.pt"):
        score_diff_tensor = torch.load("score_diff.pt")
    else:
        # Initialize an empty tensor to store score_diff values
        score_diff_tensor = torch.empty(0)
        print("Calculating score_diff_tensor")
        count=0
        for step, batch in enumerate(train_dataloader):
            if args.train_method == 'dpo':
                ...
                if args.choice_model:
                    flip, score_diff = choice_model_says_flip(batch, score_return=True)
                    score_diff_tensor = torch.cat((score_diff_tensor, score_diff.view(-1)))
                count+=1
                if count % 100 == 0:
                    print(f"Processed {count} batches")
                if count == 5000:
                    print("Breaking early for score_diff_tensor")
                    break

        print("Ended Calculating score_diff_tensor")
        file_name = "score_diff.pt"
        torch.save(score_diff_tensor, file_name)
        print(f"Saved score_diff_tensor to {file_name}")

    # Calculate the optimal threshold (\tau) and (strong and weak rewards r_h and r_l) for preference wrapping
    # This is done by finding the optimal threshold that maximizes the sum of the strong and weak rewards   
    def calculate_optimal_threshold(score_diff_tensor, method='mean'):
        if method == 'mean':
            # Calculate the optimal threshold
            tau = score_diff_tensor.mean().item()
            # Calculate the strong and weak rewards
            r_h = score_diff_tensor[score_diff_tensor > tau].mean().item()
            r_l = score_diff_tensor[score_diff_tensor <= tau].mean().item()
        elif method == 'median':
            tau = score_diff_tensor.median().item()
            # Calculate the strong and weak rewards
            r_h = score_diff_tensor[score_diff_tensor > tau].median().item()
            r_l = score_diff_tensor[score_diff_tensor <= tau].median().item()
        return tau, r_h, r_l
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                with torch.no_grad():
                    latents = vae.encode(feed_pixel_values.to(weight_dtype).to("cpu")).latent_dist.sample()
                    latents = latents.to(accelerator.device) * vae.config.scaling_factor
                ...
                if args.train_method == 'sft': # SFT, casting for F.mse_loss
                ...
                elif args.train_method == 'draft':
                    # Decode the latent representation generated by our unet the calculate the score
                    """
                    ...
                    """
                    def loss_fn(unet_pred, caption):
                        decoded_images = vae.decode(unet_pred.to(weight_dtype)).sample
                        decoded_images = (decoded_images/2 + 0.5).clamp(0,1)
                        loss = -1*(selector.diff_score(decoded_images[0], batch['caption'][0]))
                        print(loss)
                        return loss
                    
                    loss = loss_fn(model_pred, batch['caption'][0])
                    ...
                    if args.dpo_type == 'regular':
                        alpha=0
                    else:
                        alpha = args.offset_alpha
                    
                    if args.dpo_type == 'offset':
                        margin= torch.log(score_diff)
                    elif args.dpo_type == 'regular' or args.dpo_type=='beta_offset':
                        margin = torch.zeros_like(score_diff)
                    elif args.dpo_type == 'leveled_offset':
                        tau, r_h, r_l = calculate_optimal_threshold(score_diff_tensor, method='mean')
                        if args.train_batch_size == 1:
                            margin = r_h*torch.ones_like(score_diff) if score_diff > tau else r_l*torch.ones_like(score_diff) # arbitrary reward assignments for different score differences
                            # 10 should be high difference in score, 4 should be low difference in score 
                            # 1 is the threshold for a difference in score this can also be adjusted
                        else:
                            raise NotImplementedError("Leveled offset not implemented for BS>1 yet")
                        # define levels for differences in scores then apply different margins for each
                    """
                    piecewise linear beta function dependent on score_diff higher preferences get higher beta
                    Choose these threshold for score_diff values using KL divergence minimization between score distributions of different models
                    You can combine different models optimally then 
                    """
                    
                    scale_term = -0.5 * args.beta_dpo
                    if args.dpo_type=="beta_offset":
                        tau, r_h, r_l = calculate_optimal_threshold(score_diff_tensor, method='mean')
                        scale_term= 2*scale_term if score_diff>tau else scale_term
                    
                    inside_term = scale_term * (model_diff - ref_diff) + alpha * margin.to(accelerator.device)
                    # print(f"model-ref: {model_diff - ref_diff} score diff: {score_diff} margin {margin} inside term {inside_term}")
                    implicit_acc = (inside_term > 0).sum().float() / inside_term.size(0)
                    loss = -1 * F.logsigmoid(inside_term).mean() #- alpha * margin
    ...
    if accelerator.is_main_process:
        unet = accelerator.unwrap_model(unet).to("cpu")
        if args.sdxl:
            # Serialize pipeline.
            vae = AutoencoderKL.from_pretrained(
                vae_path,
