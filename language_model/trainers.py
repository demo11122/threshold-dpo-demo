...
def calculate_optimal_threshold(score_diff_tensor, method='mean'):
        if not isinstance(score_diff_tensor, torch.Tensor):
            score_diff_tensor = torch.Tensor(score_diff_tensor)
        if method == 'mean':
...
def threshold(example_tensor, threshold, high_value, low_value):

    return torch.nn.functional.threshold(example_tensor, threshold, low_value)-torch.nn.functional.threshold(-1*example_tensor, -1*threshold, -1*high_value)-example_tensor
...
def preference_loss(policy_chosen_logps: torch.FloatTensor,
                    ...
                    ipo: bool = False,
                    tdpo: bool = False, # new 
                    tdpo_high_reward: float = 0.0, # new 
                    tdpo_low_reward: float = 0.0, # new 
                    tdpo_threshold: float = 1.0, # new 
                    beta_t: bool = False, # new 
                    reference_free: bool = False,
                    offset: bool = False,
                    ratio: bool = False,
                    alpha: float = 1.,
                    chosen_rewards: torch.FloatTensor = None,
                    rejected_rewards: torch.FloatTensor = None,
                    config = None) -> Tuple[
    ...
    if ipo:
        ...
        if beta_t:
            logging.warning("using beta tdpo")
            margin = 0 * (torch.stack(chosen_rewards, dim=0) - torch.stack(rejected_rewards, dim=0)).to(logits.device)
            multiplier = threshold(torch.stack(chosen_rewards, dim=0) - torch.stack(rejected_rewards, dim=0), config.loss.threshold, config.loss.u_alpha, config.loss.l_alpha).to(logits.device)
            beta = beta * multiplier
        else:
            logging.warning("using offset tdpo")
            margin = threshold(torch.stack(chosen_rewards, dim=0) - torch.stack(rejected_rewards, dim=0), config.loss.threshold, config.loss.high_reward, config.loss.low_reward).to(logits.device)
            # margin = torch.log(margin)
            # print(margin)
...
class BasicTrainer(object):
    ...
    def calculate_tdpo_parameters(self):
        if self.config.loss.name == "tdpo":
            if self.config.loss.high_reward == "auto" or self.config.loss.low_reward == "auto" or self.config.loss.threshold == "auto":
                data_iterator_kwargs = dict(
                ...
                tdpo_iterator = get_batch_iterator(**data_iterator_kwargs, split='train',
                                                 n_examples=1024,
                                                 batch_size=1024,
                                                 silent=self.rank != 0,
                                                 cache_dir=get_local_dir(self.config.local_dirs),
                                                 loss=self.config.loss.name)
                train_batches = list(tdpo_iterator)
                # make score diffs
                score_diffs = []
                for eval_batch in (tqdm.tqdm(train_batches,
                                     desc='Calculating tdpo params...') if self.rank == 0 else sample_batches):
                    local_eval_batch = slice_and_move_batch_for_device(eval_batch, self.rank,
                                                                       self.world_size, self.rank)
                    # print(f"Prompt: {local_eval_batch['prompt']}")
                    # print(f"chosen: {local_eval_batch['chosen']}")
                    # print(f"rejected: {local_eval_batch['rejected']}")
                    chosen_rewards, _, logit_rewards = get_pos_sentiment_rewards(local_eval_batch['chosen'], self.config.datasets[0])
                    rejected_rewards, _, logit_rewards = get_pos_sentiment_rewards(local_eval_batch['rejected'], self.config.datasets[0])
                    # logging.warning(f'chosen rewards: {chosen_rewards}, rejected rewards: {rejected_rewards}')
                    score_diffs.extend([chosen_rewards[i] - rejected_rewards[i] for i in range(len(chosen_rewards))])
                    
                self.config.loss.threshold, self.config.loss.high_reward, self.config.loss.low_reward = calculate_optimal_threshold(score_diffs)
            print(f"Determining optimal threshold {self.config.loss.threshold}, high {self.config.loss.high_reward} and low reward {self.config.loss.low_reward} here")
    ...
    def get_batch_samples(self, batch: Dict[str, torch.LongTensor]) -> Tuple[str, str]:
        ...
        with ctx():
            policy_output = self.policy.generate(
                ...
                max_new_tokens=200, do_sample=True, temperature=self.config.temperature,
                top_p=self.config.topp,
                pad_token_id=self.tokenizer.pad_token_id)  # TODO: maxlen->self.config.max_length,

        if self.config.loss.name in {'dpo', 'ipo', 'odpo', 'tdpo'}:
        ...
        if self.config.loss.name in {'dpo', 'ipo', 'odpo', 'tdpo'}:
            ...
            reference_output = policy_output
            reference_output_decoded = policy_output_decoded
    ...
    def get_batch_metrics_dpo(self, metrics, train_test, policy_chosen_logps,
        ...
        losses, chosen_rewards, rejected_rewards = preference_loss(
            ...
            rejected_rewards=rejected_rewards, config=self.config)
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]],
        ...
        if loss_config.name in {'dpo', 'ipo', 'odpo', 'tdpo'}:
            ...
            if loss_config.name == 'tdpo':
                # change loss_kwargs
                # include threshold value, high & low rewards
                loss_kwargs = {'tdpo': True, 'beta': loss_config.beta, 'offset': False, 'ipo': False,
                               'alpha': loss_config.alpha, 'tdpo_high_reward': loss_config.high_reward,
                              'tdpo_low_reward': loss_config.low_reward,'tdpo_threshold': loss_config.threshold, 'beta_t': loss_config.beta_t}
                chosen_rewards, rejected_rewards = batch['chosen_reward'], batch['rejected_reward']

                #print([chosen_reward for chosen_reward in chosen_rewards])
                # print([rejected_reward for rejected_reward in rejected_rewards])
                losses, metrics = self.get_batch_metrics_dpo(metrics, train_test,
                                                             policy_chosen_logps,
                                                             policy_rejected_logps,
                                                             reference_chosen_logps,
    ...
    def calc_kl(self, batch):
        with torch.no_grad():
            ...
            # self.reference_model.to('cuda:0')
            logging.warning(
    ...
    def train(self):
        ...
        self.calculate_tdpo_parameters() # this calculates tdpo related parameters 
    ...
    def run_classifier_evals(self, policy_samples, reference_samples):
        ...
        policy_reward, policy_reward_binary, _ = get_pos_sentiment_rewards(policy_samples,
        ...
        reference_reward, reference_reward_binary, _ = get_pos_sentiment_rewards(
    ...
    def run_evals(self, example_counter=0):
        ...
        mean_eval_metrics = {k: sum(v)/(len(v)+0.000000000000000001) for k, v in self.metrics.items()}
        if self.config.wandb.enabled and self.rank == 0:
