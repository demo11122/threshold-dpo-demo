        ...
        for i in range(num_res):
            for j in range(num_res):
                ...
                if data_clean[prompt]['rewards'][i] == data_clean[prompt]['rewards'][j]:
                    ...
                    flat_corr += 1
        if flat_corr == 0:
            print("not able to construct a pair")
            data_clean.pop(prompt)
            continue
...
Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    ...
    with open(f'odpo/data/imdb-sft-{split}.jsonl', 'r') as f:
        for line in f:
...
def get_toxicity(split: str) -> Dict[
    ...
    with open(f'odpo/data/toxicity-sft-{split}-reward.jsonl', 'r') as f:
        for line in f:
            line_dict = json.loads(line)
            prompt = line_dict['prompt']
            data[prompt]['responses'].append(line_dict['text'])
...
def get_tldr(split: str) -> Dict[
    ...
    with open(f'odpo/data/tldr-sft-{split_name}-rel.json', 'r') as f:
        data = json.load(f)
    if split == 'eval':
...
def make_a_batch_ordinal(prompt, responses, rewards, prompt_tokens, response_tokens):
    ...
    for response_token in response_tokens:
        response_sequence_tokens.append({k: prompt_tokens[k] + response_token[k] for k in response_token})
        response_sequence_tokens[-1]['labels'] = response_sequence_tokens[-1]['input_ids'][:]
        response_sequence_tokens[-1]['labels'][:len(prompt_tokens['input_ids'])] = [-100] * len(prompt_tokens['input_ids'])
...
def get_batch_iterator(names: List[str],
    ...
    with TemporarilySeededRandom(seed):
        ...
        for name in names:
            ...
            for prompt, data in get_dataset(name, split, silent=silent, cache_dir=cache_dir, 
                ...
                # print(f"Printing sft target: {data['sft_target']}")
                # print(f"Printing responses: {data['responses']}")
                # print(f"Printing pairs: {data['pairs']}")
    ...
    while True:
        ...
        for prompt, responses, pairs, rewards, sft_target, truncation_mode in flat_data:
            ...
            if sft_mode:
                print(pairs)
                batch_element = tokenize_batch_element(prompt, sft_target, sft_target, rewards[pairs[0][0]], rewards[pairs[0][1]], truncation_mode, tokenizer, max_length, max_prompt_length)
